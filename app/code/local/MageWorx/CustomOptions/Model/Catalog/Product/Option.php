    ...
    public function getProductOptionCollection(Mage_Catalog_Model_Product $product) {
        ...
        if (Mage::app()->getStore()->isAdmin() && Mage::app()->getRequest()->getControllerName()=='catalog_product') {
            ...
            foreach($collection as $key => $item) {
                ...
                } elseif (!Mage::app()->getStore()->isAdmin() && $item->getViewMode()==3) {
                    $collection->removeItemByKey($key);
                    ...
                    if ($this->getGroupByType($optionType)==self::OPTION_GROUP_SELECT) {
                        if (count($option->getValues())>0) {
                            foreach ($option->getValues() as $value) {
                                ...
                                list($price, $priceType, $isSkuPrice, $taxClassId, $oldPrice, $isMsrp) = $helper->getOptionPriceAndPriceType($value->getPrice(), $value->getPriceType(), $value->getSku(), $product->getStore());
                                if ($isSkuPrice) {
                                    if ($oldPrice) {
                                        $value->setSpecialPrice($price);
                                        $value->setIsSkuPrice(1);
                                        $price = $oldPrice;
                                    } else {
                        ...
                        list($price, $priceType, $isSkuPrice, $taxClassId, $oldPrice, $isMsrp) = $helper->getOptionPriceAndPriceType($option->getPrice(), $option->getPriceType(), $option->getSku(), $product->getStore());
                        if ($isSkuPrice) {
                            if ($oldPrice) {
                                $option->setSpecialPrice($price);
                                $option->setIsSkuPrice(1);
                                $price = $oldPrice;
                            } else {
