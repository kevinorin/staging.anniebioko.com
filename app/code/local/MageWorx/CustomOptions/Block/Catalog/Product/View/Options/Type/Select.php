...
class MageWorx_CustomOptions_Block_Catalog_Product_View_Options_Type_Select extends Mage_Catalog_Block_Product_View_Options_Type_Select {
    ...
    public function getValuesHtml() {
        ...
        if ($_option->getType()==Mage_Catalog_Model_Product_Option::OPTION_TYPE_DROP_DOWN || $_option->getType()==Mage_Catalog_Model_Product_Option::OPTION_TYPE_MULTIPLE 
			...
			$selectcombine=$_option->getTitle();
		   $select = $this->getLayout()->createBlock('core/html_select')
            ...
            if ($_option->getType()==Mage_Catalog_Model_Product_Option::OPTION_TYPE_DROP_DOWN || $_option->getType()==MageWorx_CustomOptions_Model_Catalog_Product_Option::OPTION_TYPE_SWATCH) {
                if(strpos($_option->getTitle(),'Style') !== false)
				{
					$select->setName('options[' . $_option->getid() . ']')->addOption('', $this->__('Select Font Style'));
				}
				else{
					
					$select->setName('options[' . $_option->getid() . ']')->addOption('', $this->__('Choose your selection'));
				}
		 } else {
            ...
            if ($enabledDependent && $_option->getType()==Mage_Catalog_Model_Product_Option::OPTION_TYPE_MULTIPLE || $_option->getType()==MageWorx_CustomOptions_Model_Catalog_Product_Option::OPTION_TYPE_SWATCH || $_option->getType()==MageWorx_CustomOptions_Model_Catalog_Product_Option::OPTION_TYPE_MULTISWATCH) {
            ...
            foreach ($_option->getValues() as $_value) {
                ...
                if (!Mage::app()->getStore()->isAdmin() && $_value->getIsMsrp()) {
                    ...
                    if ($enabledSpecialPrice && $_value->getSpecialPrice()>0) {
                        $priceStr = ($_option->getType()==MageWorx_CustomOptions_Model_Catalog_Product_Option::OPTION_TYPE_SWATCH || $_option->getType()==MageWorx_CustomOptions_Model_Catalog_Product_Option::OPTION_TYPE_MULTISWATCH ? '<s>' : '(') . 
                                $priceStr .
                                ($_option->getType()==MageWorx_CustomOptions_Model_Catalog_Product_Option::OPTION_TYPE_SWATCH || $_option->getType()==MageWorx_CustomOptions_Model_Catalog_Product_Option::OPTION_TYPE_MULTISWATCH ? '</s> ' : ') ')  .
            ...
            } elseif ($_option->getType()==MageWorx_CustomOptions_Model_Catalog_Product_Option::OPTION_TYPE_MULTISWATCH) {
