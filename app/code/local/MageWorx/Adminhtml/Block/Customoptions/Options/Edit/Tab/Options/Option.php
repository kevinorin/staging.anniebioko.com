            ...
            foreach ($optionsArr as $optionId=>$option) {
                ...
                if ($option->getIsDelete() != '1') {
                    ...
                    if ($optionModel->getGroupByType($option->getType())==Mage_Catalog_Model_Product_Option::OPTION_GROUP_SELECT) {                        
                        ...
                        if ($countValues>0) {
                            foreach ($option->getValues() as $key => $_value) {
                                ...
                                if ($_value->getIsDelete() != '1') {
                                    ...
                                    list($price, $priceType, $isSkuPrice, $taxClassId, $oldPrice) = $helper->getOptionPriceAndPriceType(floatval(isset($storeOptionsArr[$optionId]['values'][$_value->getOptionTypeId()]['price'])?$storeOptionsArr[$optionId]['values'][$_value->getOptionTypeId()]['price']:$_value->getPrice()), isset($storeOptionsArr[$optionId]['values'][$_value->getOptionTypeId()]['price_type'])?$storeOptionsArr[$optionId]['values'][$_value->getOptionTypeId()]['price_type']:$_value->getPriceType(), $_value->getSku(), Mage::app()->getStore($this->getStoreId()));
                                    $specialPrice = '';
                                    $specialComment = '';
                        ...
                        list($price, $priceType, $isSkuPrice, $taxClassId, $oldPrice) = $helper->getOptionPriceAndPriceType(floatval(isset($storeOptionsArr[$optionId]['price'])?$storeOptionsArr[$optionId]['price']:$option->getPrice()), isset($storeOptionsArr[$optionId]['price_type'])?$storeOptionsArr[$optionId]['price_type']:$option->getPriceType(), $option->getSku(), Mage::app()->getStore($this->getStoreId()));
                        if ($isSkuPrice && $helper->isSkuPriceLinkingEnabled()) {
                            $priceDisabled = true;
                        } else {
                            $priceDisabled = false;
