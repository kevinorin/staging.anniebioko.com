...
//        }
    ...
    public function saveRewardpointsAction()
    {
        ...
        if (Mage::getStoreConfig('firecheckout/ajax_update/payment_method_on_total') // payment methods depends on grand total
            ...
            // if only one method is avaialble it should be chosen automatically
        ...
        if (!$removeGiftcard) {
            ...
            try {
                Mage::getModel('enterprise_giftcardaccount/giftcardaccount')
                    ...
                    ->removeFromCart();
                // Mage::getSingleton('checkout/session')->addSuccess(
                    // $this->__('Gift Card "%s" was removed.', Mage::helper('core')->htmlEscape($giftcardCode))
                // );
            } catch (Mage_Core_Exception $e) {
                // Mage::getSingleton('checkout/session')->addError(
                    // $e->getMessage()
...
//                    $sections[] = 'shipping-method';
    ...
    public function saveCouponAction()
    {
        ...
        if ($this->getRequest()->getPost('remove_ugiftcert', false)) {
        ...
        } elseif (($uGiftCertificate = Mage::getModel('ugiftcert/cert'))
        ...
        if ($couponChanged) {
            if ($data['code'] == $quote->getCouponCode()) {
                Mage::getSingleton('checkout/session')->addSuccess(
...
//            $billing['link_existing_account'] = $this->getRequest()->getQuery('emailexists', 0);
...
//            $quote->setCustomerComment($this->getRequest()->getPost('order-comment'));
...
//            }
            ...
            if (!isset($result['redirect']) && !isset($result['error'])) {
                ...
                if ('paypal_express' == $paymentData['method']
                    && version_compare(Mage::getVersion(), '1.6.1.0') < 0 // 1.6.1 can register customer during express checkout
        ...
        if (empty($result['error'])) {
            ...
            if ($payment && in_array($payment->getMethod(), Mage::helper('paypal/hss')->getHssMethods())) {
                ...
                $result = array(
                    'method' => 'paypalhss',
//                    'update_section' => array(
//                        'paypaliframe'  => $layout->getBlock('paypal.iframe')->toHtml()
//                    ),
                    'popup' => array(
    ...
    public function forgotpasswordAction()
    {
        ...
        if ($email) {
            if (!Zend_Validate::is($email, 'EmailAddress')) {
                ...
                if ($customer->getId()) {
                    try {
                        if (version_compare(Mage::getVersion(), '1.6.1.0') >= 0) {
